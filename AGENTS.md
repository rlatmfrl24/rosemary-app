...
- `x`: 신규 기능 추가, 화면 개선 등 메인 업데이트 수준의 변경
- `y`: 기존 기능 개선, 사용성 개선 등 마이너 업데이트 수준의 변경
- `z`: 버그 수정, 긴급 패치 등 패치 업데이트 수준의 변경

작업 완료 시 에이전트는 결과물의 성격을 자체적으로 판단해 버전 반영 여부와 증가 단위를 결정한다.

- 메인 업데이트로 판단되면 `x`를 올리고 `y`, `z`는 `0`으로 초기화한다.
- 마이너 업데이트로 판단되면 `y`를 올리고 `z`는 `0`으로 초기화한다.
- 패치 업데이트로 판단되면 `z`만 올린다.
- 앱 동작, UI, 배포 산출물에 영향이 있는 변경은 기본적으로 `package.json`의 `version`을 함께 업데이트한다.
